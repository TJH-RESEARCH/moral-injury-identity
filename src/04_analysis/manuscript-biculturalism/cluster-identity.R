...
data_cluster_id <- 
  data %>% 
    select(starts_with('wis') & 
           ends_with('total') & 
           !wis_total | civilian_commit_total
           ) %>% 
...
intern <- clValid::clValid(as.matrix(data_cluster_id), 
                           nClust =  2:6, 
...
stab <- clValid(as.matrix(data_cluster_id), 
                nClust =  2:6, 
                clMethods = c("hierarchical",
                              "kmeans",
                              "diana",
...
clusters <- hclust(dist(data_cluster_id, method = "euclidean"))
plot(clusters)
...
data <-
  data %>% 
  mutate(cluster_h_id = clusterCut2,
         cluster_h_id = factor(cluster_h_id),
         id_cluster_h_1 = ifelse(cluster_h_id == 1, 1, 0),
         id_cluster_h_2 = ifelse(cluster_h_id == 2, 1, 0)
         )
