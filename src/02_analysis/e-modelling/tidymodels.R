...
coefs <-
  bind_rows(
    fit_biis %>% broom::tidy(conf.int = TRUE, conf.level = .95) %>% mutate(model = 'biis'),
    fit_wis %>% broom::tidy(conf.int = TRUE, conf.level = .95) %>% mutate(model = 'wis'),
    fit_interact %>% broom::tidy(conf.int = TRUE, conf.level = .95) %>% mutate(model = 'interact')
  )
