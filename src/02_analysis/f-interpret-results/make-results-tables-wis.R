...
table <- 
  texreg::texreg(
    list(
      ...
      coeftest_wis_private_regard_1,
      coeftest_wis_private_regard_2
    ), 
    ci.force = T,
    ci.force.level = .95,
    ci.test = NA,
    sideways = F,
    custom.model.names = c('Attachment', 'Adjusted', 'Pride', 'Adjusted'),
    custom.coef.names = c('(Intercept)', #1
                          ...
                          'Marines',
                          'Navy'),
    caption = paste("Regressions on Military Identity Attachment and Pride"), 
    caption.above = T
  )
